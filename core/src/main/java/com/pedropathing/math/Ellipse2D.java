...
public class Ellipse2D {
    ...
    public Ellipse2D(Matrix characteristic) {
        if (characteristic.cols != 2 || characteristic.rows != 2)
        ...
        if (eigen1 < 0) throw new IllegalArgumentException("Ellipse2D must be generated by positive-definite matrix");
    ...
    public boolean outside(Vector point) {
    ...
    public Vector2D interpolate(Vector2D vector) {
        Vector2D unit = vector.normalized();
        return unit.div(Math.sqrt(unit.quadraticForm(characteristic)));
    }

    public Vector2D interpolate(double phi) { // \phi = 0 is at major axis, \phi = Math.PI/2 is at minor axis
        Vector2D unit = Vector2D.unit(theta + phi);
        return unit.div(Math.sqrt(unit.quadraticForm(characteristic)));
    }
