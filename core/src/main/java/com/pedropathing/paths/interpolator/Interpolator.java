...
public interface Interpolator {
    ...
    static Interpolator linear(double start, double end) {
        ...
        return (Curve curve, @TValue double t) -> {
            double deltaHeading = Angles.turnDirection(finalStart, finalEnd) * Angles.smallestDifference(finalStart, finalEnd);
            return Angles.normalize(finalStart + deltaHeading * t);
        };
    }

    static Interpolator facingPose(Pose pose) {
        return (Curve curve, @TValue double t) -> pose.toVector2D().minus(curve.get(t)).theta();
    }
