...
public class Swerve extends CustomDrivetrain {
    ...
    public Swerve(HardwareMap hardwareMap, SwerveConstants constants) {
        ...
        leftFrontPod = new SwervePod(
                ...
                constants.getLeftFrontTurnPID(), constants.getLeftFrontMotorDirection(),
                constants.getLeftFrontServoDirection(), constants.getLeftFrontPodAngleOffsetDeg(),
                constants.getLeftFrontPodXYOffsets(), constants.getLeftFrontReferenceVoltage(),
                constants.getLeftFrontEncoderReversed());
        ...
        rightFrontPod = new SwervePod(
                ...
                constants.getRightFrontTurnPID(), constants.getRightFrontMotorDirection(),
                constants.getRightFrontServoDirection(), constants.getRightFrontPodAngleOffsetDeg(),
                constants.getRightFrontPodXYOffsets(), constants.getRightFrontReferenceVoltage(),
                constants.getRightFrontEncoderReversed());
        ...
        leftRearPod = new SwervePod(
                ...
                constants.getLeftRearTurnPID(), constants.getLeftRearMotorDirection(),
                constants.getLeftRearServoDirection(), constants.getLeftRearPodAngleOffsetDeg(),
                constants.getLeftRearPodXYOffsets(), constants.getLeftRearReferenceVoltage(),
                constants.getLeftRearEncoderReversed());
        ...
        rightRearPod = new SwervePod(
                ...
                constants.getRightRearTurnPID(), constants.getRightRearMotorDirection(),
                constants.getRightRearServoDirection(), constants.getRightRearPodAngleOffsetDeg(),
                constants.getRightRearPodXYOffsets(), constants.getRightRearReferenceVoltage(),
                constants.getRightRearEncoderReversed());
     ...
     * Intended to work exactly like an arcade drive would in a typical TeleOp, this method can be a
     * copy pasted from a robot-centric arcade drive implementation.
     *
     * @param forward the forward power value, which would typically be -gamepad1.left_stick_y in a
     *        normal arcade drive setup.
     * @param strafe the strafe power value, which would typically be gamepad1.left_stick_x in a
     *        normal arcade drive setup.
     * @param rotation the rotation power value, which would typically be gamepad1.right_stick_x in
     *        a normal arcade drive setup.
     */
    public void arcadeDrive(double forward, double strafe, double rotation) {
        ...
        // of 1
        Vector rawTrans = new Vector(Range.clip(Math.hypot(strafe, forward), 0, 1),
                Math.atan2(forward, strafe));
        ...
        for (int i = 0; i < pods.length; i++) {
            ...
            Vector rotationVector =
                    new Vector(rotationScalar, Math.atan2(pod.getYOffset(), pod.getXOffset()));
            ...
            if (constants
                    .getZeroPowerBehavior() == SwerveConstants.ZeroPowerBehavior.RESIST_MOVEMENT
        ...
        for (int podNum = 0; podNum < pods.length; podNum++) {
            ...
            pods[podNum].move(finalVector.getTheta(), finalVector.getMagnitude() * avgScaling,
                    zeroTrans && zeroRotation && constants
                            .getZeroPowerBehavior() == SwerveConstants.ZeroPowerBehavior.IGNORE_ANGLE_CHANGES,
                    motorCachingThreshold, servoCachingThreshold, drivePIDFFeedForward);
        }
    }
    ...
    public void updateConstants() {
        ...
        this.maxPowerScaling = constants.getMaxPower(); // inherited from Drivetrain, used by
                                                        // CustomDrivetrain
        ...
        this.voltageCompensation = constants.getUseVoltageCompensation(); // inherited from
                                                                          // Drivetrain
    ...
    public String debugString() {
        return "Swerve{" + "\n leftFrontPodAngle = " + leftFrontPod.getAngleAfterOffsetDeg()
                ...
                + "\n, rotation input=" + lastRotation + "\n, unrotated translationVector x"
                + lastTranslationalVector.getXComponent() + "\n, unrotated translationVector y"
                + lastTranslationalVector.getYComponent() + "\n, correctivePower x"
                + lastCorrectivePower.getXComponent() + "\n, correctivePower y"
                + lastCorrectivePower.getYComponent() + "\n, pathingPower x"
                + lastPathingPower.getXComponent() + "\n, pathingPower y"
                + lastPathingPower.getYComponent() + "\n, headingPower magnitude"
                + lastHeadingPower.getMagnitude() + "\n, headingPower direction"
                + lastHeadingPower.getTheta() + "\nrobot heading" + lastHeading + "\navg scaling"
                + lastAvgScaling + "\n, leftFront=" + leftFrontPod.debugString() + "\n, rightFront="
                + rightFrontPod.debugString() + "\n, leftRear=" + leftRearPod.debugString()
